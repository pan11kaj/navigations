...
constructor(){
    super();
    this.state={
        email:'',password:'',id:'',visibility:false
    }
}
Login=(e,p)=>{
    db.collection('students').where('student_id','==',this.state.id)
    ...
    db.collection('teachers').where('teacher_id','==',this.state.id)
    .where('email','==',this.state.email)
    .get()
    .then(()=>{
        firebase.auth().signInWithEmailAndPassword(e,p)
        .then(response=>{
           return this.props.navigation.navigate('tab2')
        })
        .catch((error)=>{
            var errcode = error.code;
            var message = error.message;
